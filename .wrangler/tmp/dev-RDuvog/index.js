...
var getDB = /* @__PURE__ */ __name(async (c) => {
  try {
    const data = await c.env.PHOTO_DB.get("index.json");
    if (!data) {
      console.error("No data found in PHOTO_DB for key 'index.json'");
...
app.get("/api/categories", async (c) => {
  ...
  let categories = db.groups.map((g) => {
    return g.category;
  });
...
// .wrangler/tmp/bundle-3aWCpy/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-3aWCpy/middleware-loader.entry.ts
